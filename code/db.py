...
class Database:
    ...
    def create_tables(self):
        ...
        self.conn.commit()
        self._ensure_favorites_meal_id()

    def _ensure_favorites_meal_id(self):
        """Миграция: внешний id рецепта TheMealDB для связи с API."""
        try:
            self.cursor.execute("ALTER TABLE favorites ADD COLUMN meal_id TEXT")
            self.conn.commit()
    ...
    def load_favorites(self):
        self.cursor.execute('SELECT name, ingredients, instructions, time, meal_id FROM favorites')
        rows = []
    ...
    def save_favorites(self, items):
        ...
        for item in items:
            self.cursor.execute(
                'INSERT INTO favorites (name, ingredients, instructions, time, meal_id) VALUES (?, ?, ?, ?, ?)',
                (
                    item['name'],
                    item['ingredients'],
                    item['instructions'],
